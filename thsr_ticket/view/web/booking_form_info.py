...
from configs.common import DAYS_BEFORE_BOOKING_AVAILABLE
from model.web.booking_form.station_mapping import StationMapping
from model.web.booking_form.ticket_num import BaseTicket
from model.web.booking_form.time_table import TimeTable
...
class BookingFormInfo:
    ...
    def station_info(
        self, station_type: str, default_value: int = None, select: bool = True
    ) -> int:
        ...
        if select:
            return int(
                input("輸入選擇(預設: {}): ".format(str(default_value)))
                or default_value
            )
        return None
    ...
    def ticket_num_info(
        self, ticket_type: str, default_value: int = 0, select: bool = True
    ) -> int:
        max_num = len(BaseTicket()) - 1
        print(
            "選擇{}票數 ({}~{}, 預設: {}): ".format(
                ticket_type, 0, max_num, default_value
            )
        )
        if select:
    ...
    def time_table_info(self, default_value: int = None, select: bool = True) -> str:
        ...
        if select:
            return self.time_table.get_time(
                int(
                    input("輸入選擇(預設: {}): ".format(default_value)) or default_value
                )
            )
        return None
